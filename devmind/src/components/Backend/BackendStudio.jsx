...
export default function BackendStudio() {
  ...
  const handleRun = async () => {
    if (!selectedLang.live) {
      setOutput({
        output: "",
        error: `Live execution for ${selectedLang.label} is coming soon. Copy the code and run it locally.`,
      });
      setActiveTab("output");
      return;
    }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950 text-white font-mono">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-900 border-b border-gray-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleRun}
            disabled={running}
            className={`px-3 py-1 text-white text-xs rounded-lg transition-all disabled:opacity-50 ${
              selectedLang.live
                ? "bg-green-700 hover:bg-green-600"
                : "bg-gray-700 hover:bg-gray-600"
            }`}
          >
            {running ? "⏳ Running..." : selectedLang.live ? "▶ Run" : "▶ Run locally"}
          </button>
        </div>
      </div>
      ...
      {/* Live/Local info banner */}
      {!selectedLang.live && (
        <div className="px-4 py-2 bg-gray-900/50 border-b border-gray-800 flex items-center gap-2">
          <span className="text-gray-500 text-xs">
            📋 {selectedLang.label} runs locally. Use the AI to generate code, copy it, and run it on your machine.
          </span>
        </div>
      )}
          ...
          {!output ? (
            <p className="text-gray-600 text-sm">
              {selectedLang.live
                ...
                : `Copy the code and run it locally with ${selectedLang.label}.`}
            </p>
          ) : (
              ...
              {output.error && (
                <div>
                  <p className={`text-xs mb-2 font-bold ${
                    output.error.includes("locally") ? "text-yellow-400" : "text-red-400"
                  }`}>
                    {output.error.includes("locally") ? "ℹ️ INFO:" : "❌ ERROR:"}
                  </p>
                  <pre className={`text-sm bg-gray-900 p-3 rounded-lg whitespace-pre-wrap ${
                    output.error.includes("locally") ? "text-yellow-300" : "text-red-300"
                  }`}>
                    {output.error}
                  </pre>
                </div>
      ...
      {githubStatus && (
        <div className={`px-4 py-3 border-t border-gray-800 text-xs ${
          ...
          {githubStatus.success ? (
            <>✅ Pushed! <a href={githubStatus.url} target="_blank" rel="noreferrer" className="underline">{githubStatus.url}</a></>
          ) : (
